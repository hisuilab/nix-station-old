...
| 5 | 高 | `tmp/packages.json` を `hosts/windows-desktop/packages/` 以下にカテゴリ別に分割して正式管理する。構成: `gaming.json`（Steam・Epic・OBS・PlayStation・MSI）/ `dev.json`（WSL・Ubuntu・Terminal・Docker・VSCode・Tailscale）/ `hardware.json`（Logitech・Yamaha・Nvidia PhysX）/ `browser.json`（Chrome・Brave）。`setup.ps1` が各ファイルを順番に `winget import` する | 用途混在の単一ファイルでは追加・削除時にどのカテゴリへの変更か分かりにくい。カテゴリ別ファイルにより「ゲームだけ入れる」「開発環境だけ後から追加」の選択的インストールが可能になる |
